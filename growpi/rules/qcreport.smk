...
if config["params"]["qcreport"]["do"]:
    rule qcreport_summary:
        ...
        run:
            df = growpi.merge(input, growpi.parse, threads)
            growpi.compute_host_rate(df, STEPS, SAMPLES_ID_LIST, allow_miss_samples=True, output=output[0])
    ...
    rule qcreport_plot:
        ...
        priority:
            30
        run:
            df = growpi.parse(input[0])
            growpi.qc_bar_plot(df, "seaborn", output=output[0])
