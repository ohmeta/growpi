def raw_short_reads(wildcards):
    if READS_FORMAT == "fastq":
        if config["params"]["simulate"]["do"]:
            return [growpi.get_reads(SAMPLES, wildcards, "fq1")[0],
                    growpi.get_reads(SAMPLES, wildcards, "fq2")[0]]
        else:
            if IS_PE:
                if IS_INTERLEAVED:
                    return [growpi.get_reads(SAMPLES, wildcards, "fq1")]
                else:
                    return [growpi.get_reads(SAMPLES, wildcards, "fq1"),
                            growpi.get_reads(SAMPLES, wildcards, "fq2")]
            else:
                return [growpi.get_reads(SAMPLES, wildcards, "fq1")]
    elif READS_FORMAT == "sra":
        return [growpi.get_reads(SAMPLES, wildcards, "sra")]


def raw_long_reads(wildcards):
    if READS_FORMAT == "fastq":
        return [growpi.get_reads(SAMPLES, wildcards, "fq_long")]
    else:
...
if config["params"]["qcreport"]["do"]:
    ...
    rule raw_report_refine:
        ...
        run:
            if IS_PE:
                growpi.change(str(input), str(output), params.sample_id, "raw",
                              "pe", ["fq1", "fq2"])
            else:
                growpi.change(str(input), str(output), params.sample_id, "raw",
                              "se", ["fq1"])
    ...
    rule raw_report_merge:
        ...
        threads:
            config["params"]["qcreport"]["seqkit"]["threads"]
        run:
            growpi.merge(input, growpi.parse, threads, output=output[0])
