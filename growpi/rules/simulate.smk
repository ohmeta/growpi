if config["params"]["simulate"]["do"]:
    rule simulate_short_reads:
        input:
            genomes = lambda wildcards: growpi.get_simulate_info(SAMPLES, wildcards, "genome")
        output:
        ...
        params:
            output_prefix = os.path.join(config["output"]["simulate"],
                                         "short_reads/{sample}"),
            model = lambda wildcards: growpi.get_simulate_info(SAMPLES, wildcards, "model")[0],
            reads_num = lambda wildcards: growpi.get_simulate_info(SAMPLES, wildcards, "reads_num")[0],
            abundance = lambda wildcards: growpi.get_simulate_info(SAMPLES, wildcards, "abundance")
        threads:
            config["params"]["simulate"]["threads"]
        run:
            growpi.simulate_short_reads(input.genomes,
